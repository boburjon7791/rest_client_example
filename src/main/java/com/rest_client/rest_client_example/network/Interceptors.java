...
class Interceptors {
    ...
    static ClientHttpRequestInterceptor loggingInterceptor(){
    ...
    static ClientHttpRequestInterceptor catApiInterceptor(){
        return (request, body, execution) -> {
            if (request.getURI().equals(URI.create(RestNetwork.CAT_IMAGE_UPLOAD))){
                request.getHeaders().set(RestNetwork.X_API_KEY, RestNetwork.CAT_API_KEY);
            }
